...
export default function AccesSousTraitant({ sousTraitantId, hasAccess, email }: Props) {
  ...
  async function create() {
    ...
    setBusy('create')
    try {
      const res = await fetch(`/api/sous-traitants/${sousTraitantId}/acces`, {
        method: 'POST',
  ...
  async function resetPassword() {
    setBusy('reset')
    try {
      const res = await fetch(`/api/sous-traitants/${sousTraitantId}/acces`, { method: 'PATCH' })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error ?? 'Erreur')
      setCreds(json.data)
  ...
  async function revoke() {
    if (!confirm("Révoquer l'accès de ce sous-traitant ? Son compte sera supprimé.")) return
    setBusy('revoke')
    try {
      const res = await fetch(`/api/sous-traitants/${sousTraitantId}/acces`, { method: 'DELETE' })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error ?? 'Erreur')
      setCreds(null)
  ...
  function copyCreds() {
    ...
    navigator.clipboard.writeText(
      `Application Star Tourism Services — Sous-traitant\n${appUrl}\n\nEmail : ${creds.email}\nMot de passe : ${creds.password}`
    ).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000) })
  }

  return (
    <div className="card" style={{ marginBottom: '16px' }}>
      <div className="card-header"><span className="card-header-title">Accès application sous-traitant</span></div>
        ...
        {hasAccess ? (
          ...
          <div>
            ...
            <input type="email" className="input" value={formEmail} onChange={e => setFormEmail(e.target.value)}
              placeholder="contact@sous-traitant.fr" style={{ marginBottom: '10px' }} />
          ...
          <>
            <p style={{ fontSize: '11px', color: '#5a564e', lineHeight: 1.6, marginBottom: '12px' }}>
              Donnez à ce sous-traitant l'accès à l'application mobile : il verra les missions que vous lui confiez
              (adresses, passagers, vol, contact client), pourra changer le statut et saisir les heures.
            </p>
            <button onClick={() => setShowForm(true)} className="btn-primary" style={{ width: '100%', justifyContent: 'center', gap: '8px' }}>
              <KeyRound size={14} /> Créer l'accès sous-traitant
            </button>
          </>
        )}
      </div>
