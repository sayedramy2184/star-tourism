  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <Link href="/dashboard/paie" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#63605a', textDecoration: 'none' }}>
      ...
      {paie.dossiers.length === 0 ? (
        <div className="card" style={{ padding: '30px', textAlign: 'center', color: '#63605a', fontSize: '13px' }}>
          ...
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {paie.versementsHorsDossier.map(p => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '7px 10px', background: '#faf9f7', border: '1px solid #f4f5f7' }}>
                ...
                <span style={{ fontSize: '10px', color: '#63605a' }}>{format(parseISO(p.date_paiement), 'dd/MM/yyyy', { locale: fr })}</span>
                {p.moyen && <span style={{ fontSize: '9px', color: '#5a564e', textTransform: 'uppercase' }}>{p.moyen}</span>}
                {p.note && <span style={{ fontSize: '10px', color: '#63605a' }}>· {p.note}</span>}
              </div>
            ))}
          </div>
        </div>
